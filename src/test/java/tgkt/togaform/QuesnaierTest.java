...
public class QuesnaierTest {
    ...
    void query() {
        ...
        q.setId("1111");//已存在
        Assertions.assertFalse(((List<Quesnaire>)
        ...
        q.setId(null);
        Assertions.assertTrue(((List<Quesnaire>)
                        (controller.queryById(q)
                                .getData()))
                        .isEmpty()
    ...
    void add() {
        ...
        log.info("Quesnaire模块>>添加请求测试2：工程名为null，通过");
    ...
    void modify() {
        var q = new QuesnaireListRequest();
        q.setProject("guagua");//已存在
        q = (QuesnaireListRequest) ((List<?>) (controller.queryByProject(q).getData())).get(0);
